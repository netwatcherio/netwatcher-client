...
<template>
  <div class="code-copy label-code d-flex gap-1 align-items-center code-field" @click="() => copyText()"
     >
    <div v-if="props.title" style="width: 2.5rem" class="label-c5 text-primary px-1">{{ props.title }}</div>
    <div class="d-flex gap-1 justify-content-between w-100">
      <div class="label-o4 flex-grow-1">
        {{ state.displayText }}
      </div>
      <div class="d-flex gap-1">
        <span class="copy-box "><i v-if="!state.copied" class="bi bi-clipboard"></i><i v-else
                                                                                       class="bi bi-clipboard-check copied"></i></span>
